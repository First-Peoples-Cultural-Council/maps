...
def send_claim_profile_invite(user_email, email_data):
    ...
    message = """
        ...
        <p>&emsp;<a href='{host}/art/{profile}' target='_blank'>{host}/art/{profile}</a></p><br/>
    ...
    """.format(
        ...
        host=settings.HOST,
        profile=_format_fpcc(email_data.placename.name),
    ...
    send_mail(
        subject="Claim FPCC Profile: %s" % email_data.placename.name,
        message=message,
    ...
    try:
        record = ProfileClaimRecord.objects.get(profile_email=email_data.value)
        record.key = key
        record.user_email = user_email
        record.save()
    except ProfileClaimRecord.DoesNotExist:
        ProfileClaimRecord.objects.create(
            profile_email=email_data.value,
...
def send_claim_profile_invites():
    ...
    related_emails = RelatedData.objects.exclude(value='').filter(
...
def resend_invite(user_email):
    ...
    """
    record = ProfileClaimRecord.objects.get(user_email=user_email)
    email_data = RelatedData.objects.get(data_type='email', value=record.profile_email)
