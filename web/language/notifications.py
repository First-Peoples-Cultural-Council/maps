...
def notify(user, since=None):
    ...
    if user_is_admin:
        ...
        intro.append(
            '<p>This is a test-only setup. We are not expecting that many updates, so even if the updates are for every single language and community, we will not be overwhelemed.'.format(
                ','.join([get_lang_link(l) for l in languages])
            )
        )
    else:
