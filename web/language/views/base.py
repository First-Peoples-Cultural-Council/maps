...
def get_queryset_for_user(view, request):
    ...
    filter_media = (
        user_logged_in and queryset.model is Media and not request.GET.get("placename")
    )
    ...
    if filter_media or filter_placenames:
        ...
        user_communities = (
            CommunityMember.objects.filter(user__id=int(request.user.id))
            .filter(status__exact=VERIFIED)
            .values("community")
        )
        ...
        if filter_placenames:
            filter_query = (
                ...
                | Q(status__in=[FLAGGED, REJECTED], communities__in=user_communities)
            )
            exclude_query = Q(community_only=True) & ~Q(
        ...
        queryset_community = queryset.filter(filter_query).exclude(exclude_query)
        ...
        queryset = queryset.exclude(
            Q(community_only=True) | Q(status=FLAGGED) | Q(status=REJECTED)
        )
...
def _filter_media_queryset_for_admin(
    ...
    qry_adm_places = queryset.filter(
        placename__language__in=admin_languages,
        placename__communities__in=admin_communities,
    )
    if qry_adm_community and qry_adm_places:
        queryset = (
            queryset_user.union(queryset_community)
            .union(qry_adm_community)
            .union(qry_adm_places)
        )
